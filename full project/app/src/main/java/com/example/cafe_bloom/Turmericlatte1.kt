...
class Turmericlatte1 : AppCompatActivity() {

    private var quantity = 1
    private var sizePrice = 1400
    private var toppingPrice = 0
    private var chocolatePrice = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.turmericlatte)
        ...
        btnSmall.setOnClickListener {
            selectSize(btnSmall, btnMedium, btnLarge, 1400, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnMedium.setOnClickListener {
            selectSize(btnMedium, btnSmall, btnLarge, 1500, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnLarge.setOnClickListener {
            selectSize(btnLarge, btnSmall, btnMedium, 1600, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
    ...
    private fun updateSelectedItems(tvSelectedItems: TextView) {
        ...
        selectedItems.append("Size: ${if (sizePrice == 1400) "Small" else if (sizePrice == 1500) "Medium" else "Large"}\n")

        if (chocolatePrice > 0) {
            selectedItems.append("Type of Milk: ${if (chocolatePrice == 100) "Whole Milk" else if (chocolatePrice == 80) "Skim Milk" else "Half-and-Half"}\n")
        }

        if (toppingPrice > 0) {
            selectedItems.append("Topping: ${if (toppingPrice == 50) "Crushed Pistachios" else "Cinnamon Dusting"}\n")
        }
