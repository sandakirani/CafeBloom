...
class Tropicalfruit1 : AppCompatActivity() {

    private var quantity = 1
    private var sizePrice = 2000
    private var toppingPrice = 0
    private var chocolatePrice = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.tropicalfruit)
        ...
        btnSmall.setOnClickListener {
            selectSize(btnSmall, btnMedium, btnLarge, 2000, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnMedium.setOnClickListener {
            selectSize(btnMedium, btnSmall, btnLarge, 2200, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnLarge.setOnClickListener {
            selectSize(btnLarge, btnSmall, btnMedium, 2400, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
    ...
    private fun updateSelectedItems(tvSelectedItems: TextView) {
        ...
        selectedItems.append("Size: ${if (sizePrice == 2000) "Small" else if (sizePrice == 2200) "Medium" else "Large"}\n")

        if (chocolatePrice > 0) {
            selectedItems.append("Accompaniments: ${if (chocolatePrice == 100) "Granola " else if (chocolatePrice == 80) "Mint Leaves" else "Chia Seeds"}\n")
        }

        if (toppingPrice > 0) {
            selectedItems.append("Add-Ons: ${if (toppingPrice == 50) "Almond Slivers " else "Dark Chocolate Drizzle"}\n")
        }
