...
class Seafood1 : AppCompatActivity() {

    private var quantity = 1
    private var sizePrice = 2500
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSmall.setOnClickListener {
            selectSize(btnSmall, btnMedium, btnLarge, 2500, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnMedium.setOnClickListener {
            selectSize(btnMedium, btnSmall, btnLarge, 2550, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnLarge.setOnClickListener {
            selectSize(btnLarge, btnSmall, btnMedium, 3000, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
    ...
    private fun updateSelectedItems(tvSelectedItems: TextView) {
        ...
        selectedItems.append("Size: ${if (sizePrice == 2500) "Small" else if (sizePrice == 2550) "Medium" else "Large"}\n")

        if (chocolatePrice > 0) {
            selectedItems.append("Type of Roti: ${if (chocolatePrice == 100) "Garlic Roti" else if (chocolatePrice == 80) "Whole Wheat Roti" else "Plain Roti"}\n")
        }
