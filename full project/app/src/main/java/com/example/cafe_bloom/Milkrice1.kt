...
class Milkrice1 : AppCompatActivity() {

    private var quantity = 1
    private var sizePrice = 1600
    private var toppingPrice = 0
    private var chocolatePrice = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.milkrice)
        ...
        btnSmall.setOnClickListener {
            selectSize(btnSmall, btnMedium, btnLarge, 1600, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnMedium.setOnClickListener {
            selectSize(btnMedium, btnSmall, btnLarge, 1650, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnLarge.setOnClickListener {
            selectSize(btnLarge, btnSmall, btnMedium, 1700, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
    ...
    private fun updateSelectedItems(tvSelectedItems: TextView) {
        ...
        selectedItems.append("Size: ${if (sizePrice == 1600) "Small" else if (sizePrice == 1650) "Medium" else "Large"}\n")

        if (chocolatePrice > 0) {
            selectedItems.append("Accompaniments: ${if (chocolatePrice == 100) "Chicken Curry" else if (chocolatePrice == 80) "Beef Curry" else "Coconut Sambol"}\n")
        }
