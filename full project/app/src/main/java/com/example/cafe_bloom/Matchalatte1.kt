...
class Matchalatte1 : AppCompatActivity() {

    private var quantity = 1
    private var sizePrice = 1500
    private var toppingPrice = 0
    private var chocolatePrice = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.matchalatte)
        ...
        btnSmall.setOnClickListener {
            selectSize(btnSmall, btnMedium, btnLarge, 1500, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnMedium.setOnClickListener {
            selectSize(btnMedium, btnSmall, btnLarge, 1590, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnLarge.setOnClickListener {
            selectSize(btnLarge, btnSmall, btnMedium, 1680, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
    ...
    private fun updateSelectedItems(tvSelectedItems: TextView) {
        ...
        selectedItems.append("Size: ${if (sizePrice == 1500) "Small" else if (sizePrice == 1590) "Medium" else "Large"}\n")

        if (chocolatePrice > 0) {
            selectedItems.append("Sweetener: ${if (chocolatePrice == 100) "Honey" else if (chocolatePrice == 80) "Cane Sugar" else "Date Syrup"}\n")
        }

        if (toppingPrice > 0) {
            selectedItems.append("Topping: ${if (toppingPrice == 50) "Whipped Cream" else "Honey Drizzle"}\n")
        }
