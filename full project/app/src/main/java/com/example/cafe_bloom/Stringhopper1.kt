...
class Stringhopper1 : AppCompatActivity() {

    private var quantity = 1
    private var sizePrice = 500
    private var toppingPrice = 0
    private var chocolatePrice = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.stringhopper)
        ...
        btnSmall.setOnClickListener {
            selectSize(btnSmall, btnMedium, btnLarge, 500, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnMedium.setOnClickListener {
            selectSize(btnMedium, btnSmall, btnLarge, 550, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
        btnLarge.setOnClickListener {
            selectSize(btnLarge, btnSmall, btnMedium, 600, tvTotalPrice)
            updateSelectedItems(tvSelectedItems)
        }
    ...
    private fun updateSelectedItems(tvSelectedItems: TextView) {
        ...
        selectedItems.append("Size: ${if (sizePrice == 500) "Small" else if (sizePrice == 550) "Medium" else "Large"}\n")

        if (chocolatePrice > 0) {
            selectedItems.append("Additional Enhancements: ${if (chocolatePrice == 100) "Avocado Slices" else if (chocolatePrice == 80) "Scrambled Egg" else "Poached Egg"}\n")
        }

        if (toppingPrice > 0) {
            selectedItems.append("Spice Level: ${if (toppingPrice == 50) "Mild " else "Spicy"}\n")
        }
